    ...
    try {
      if (!needsTrim && !needsVideoTranscode && !needsAudioTranscode) {
        ...
        final videoFilter = isVertical
            ? 'scale=1080:1920:flags=bicubic,fps=30,format=yuv420p'
            : (_shortsStyle == 'crop'
                ? 'scale=1080:1920:force_original_aspect_ratio=increase:flags=bicubic,crop=1080:1920,fps=30,format=yuv420p'
                : (_shortsStyle == 'black'
                    ? 'scale=1080:-2:flags=bicubic,pad=1080:1920:(ow-iw)/2:(oh-ih)/2:color=black,fps=30,format=yuv420p'
                    : '[0:v]scale=1080:1920:force_original_aspect_ratio=increase:flags=bicubic,boxblur=20:5,eq=brightness=-0.15:contrast=0.95[bg];[0:v]scale=1080:-2:flags=bicubic[fg];[bg][fg]overlay=(W-w)/2:(H-h)/2,fps=30,format=yuv420p'));
